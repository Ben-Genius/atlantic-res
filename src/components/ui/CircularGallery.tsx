...
class App {
  ...
  constructor(
    ...
    this.disableInternalScroll = disableInternalScroll;
    this.isInfinite = isInfinite;
  ...
  createMedias(
    ...
    this.mediasImages = this.isInfinite ? galleryItems.concat(galleryItems) : galleryItems;
  ...
  onCheck() {
    if (!this.medias || !this.medias[0] || this.isInfinite === false) return;
...
const CircularGallery = forwardRef<CircularGalleryRef, CircularGalleryProps>(
  (
    ...
    useImperativeHandle(ref, () => ({
      setScroll: (progress: number) => {
        if (appRef.current) {
          appRef.current.setScroll(progress);
        }
      }
    }));
